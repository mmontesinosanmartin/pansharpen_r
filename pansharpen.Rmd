...
The function `mark()` from the `bench` package () tracks the running
times in `R` . The argument `time_unit = "m"` ensures consistent time
units (here minutes) across tests. We run the three pansharpening
...
```{r saga_test}
bro.saga <- mark(pansharpen_brovey(saga.path, ms.lrg, pan.lrg), time_unit = "m")
ihs.saga <- mark(pansharpen_ihs(saga.path, ms.lrg, pan.lrg), time_unit = "m")
pca.saga <- mark(pansharpen_pca(saga.path, ms.lrg, pan.lrg), time_unit = "m")
```
